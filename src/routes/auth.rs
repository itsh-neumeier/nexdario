    ...
    match user_row {
        Ok(Some(row)) if row.is_active != 0 => {
            if auth_utils::verify_password(&form.password, &row.password_hash) {
                ...
                audit::log_login(&state.db, row.id.unwrap_or(0), &row.username, ip.as_deref(), true).await;
                ...
                // Log failed attempt
                audit::log_login(&state.db, row.id.unwrap_or(0), &row.username, ip.as_deref(), false).await;
